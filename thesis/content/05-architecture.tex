...
The on-chain contracts are deterministic and inspectable, and we treat them as trusted once deployed. The trace validator and the \textit{RequiredEvidence} schemas are authored, content-addressed, and version-pinned, and we treat them as trusted under their documented key model. The language model in the proposer is opaque, replaceable, and not trusted to be correct. External evidence sources are untrusted in the worst case and lossy in the best case. Human approvers are trusted to authorise the actions they intend, but they are not assumed to be alert at three in the morning.

The threat register below names the threats the design contains, marks the trust status of the affected component, and points to the mechanism that holds the line. Each entry is referenced from at least one invariant in Section \ref{sec:proof-targets}, so the register is load-bearing rather than decorative.

\begin{description}[leftmargin=1.2cm,labelwidth=0.9cm,style=nextline,itemsep=0.85\baselineskip]
\item[\textbf{T1}] \textbf{Poisoned evidence.} A source returns crafted content intended to flip the decision.
\textit{Trust status:} evidence sources are untrusted.
\textit{Containing mechanism:} the schema requires multi-source corroboration and provenance per category. The trace validator rejects traces that rely on a single uncorroborated source for a decision-critical category. Hard constraints catch out-of-range values.

\item[\textbf{T2}] \textbf{Stale evidence.} Data was fresh when retrieved but is no longer fresh at authorisation time.
\textit{Trust status:} evidence sources are untrusted, and the trace validator is trusted.
\textit{Containing mechanism:} per-category freshness time-to-live values (TTLs) in the \textit{RequiredEvidence} schema are evaluated at authorisation time $t$, not at trace construction time. Section \ref{sec:proof-targets} states this as part of the Trace-Gated Authorisation property.

\item[\textbf{T3}] \textbf{Prompt injection into the proposer.} Instructions embedded in retrieved evidence attempt to override the proposer's role.
\textit{Trust status:} the language model and retrieved text are untrusted.
\textit{Containing mechanism:} the proposer cannot emit a final decision. It can only emit a trace structured to the trace validator schema. Out-of-schema fields and free-text recommendations are dropped. The trace validator performs the gating check on structured fields, not on natural-language output.

\item[\textbf{T4}] \textbf{Malicious or vague intent.} A stated intent does not correspond to the action being prepared, or is too underspecified for a confident decision.
\textit{Trust status:} intent comes from a trusted human but is not assumed correct.
\textit{Containing mechanism:} intent is recorded as a structured input that drives the projection $P_i$. Underspecified intents fail schema requirements and force \textit{defer}. The transaction preparation step checks that the prepared transaction matches the trace, which in turn is bound to the recorded intent.

\item[\textbf{T5}] \textbf{Manipulated oracles.} Price or risk feeds report manipulated values.
\textit{Trust status:} oracles are untrusted.
\textit{Containing mechanism:} the schema specifies the oracle set per category, requires cross-feed agreement within a tolerance for decision-critical categories, and records each feed's provenance in the trace. NFTAA policy can additionally cap notional per action.

\item[\textbf{T6}] \textbf{Incomplete traces.} A trace omits a required evidence category, or its provenance is missing.
\textit{Trust status:} the proposer is untrusted, and the trace validator is trusted.
\textit{Containing mechanism:} Evidence-Completeness Safety in Section \ref{sec:proof-targets}. The trace validator refuses a confident \textit{proceed} unless \textit{RequiredEvidence}$(i, a) \subseteq E$ and provenance is recorded for every entry in $E$.

\item[\textbf{T7}] \textbf{Transaction mismatch.} A candidate transaction is submitted that does not correspond to the approved trace.
\textit{Trust status:} transaction preparation is trusted-but-verified, and the signer environment is untrusted.
\textit{Containing mechanism:} the approval signature covers both the trace hash and the transaction hash. NFTAA verifies that the submitted transaction hashes to the value bound to the approval, and rejects otherwise.

\item[\textbf{T8}] \textbf{Compromised agent process.} The off-chain agent process is corrupted and emits arbitrary traces.
\textit{Trust status:} the off-chain runtime is untrusted at the boundary.
\textit{Containing mechanism:} authority does not flow through the agent. The agent cannot authorise action. Only a human-approved, trace-validator-signed trace bound to a policy-compliant transaction settles on-chain. A compromised agent can produce invalid traces, which the trace validator and NFTAA reject.

\item[\textbf{T9}] \textbf{Policy bypass at NFTAA.} An attempt is made to settle a value-relevant action without passing the on-chain policy.
\textit{Trust status:} the NFTAA contract is trusted.
\textit{Containing mechanism:} the NFTAA contract is the sole settlement path for value-relevant actions of the account, and enforces the trace-gated authorisation checks of Section \ref{sec:proof-targets}. Direct asset transfers that bypass NFTAA are out of scope of the account's policy and out of scope of the safety claims.

\item[\textbf{T10}] \textbf{Schema authorship attack.} A weakened or replaced \textit{RequiredEvidence} schema lowers the bar for confident decisions.
\textit{Trust status:} schemas are trusted authored artefacts.
\textit{Containing mechanism:} schemas are versioned and content-addressed. The trace validator signs the trace jointly with the schema version and schema hash. NFTAA only accepts traces whose schema version is in a recognised set. Replacing the schema therefore breaks the signature rather than silently weakening the bar.
\end{description}

Three assumptions sit outside the register because they bound the model rather than describe a threat. We assume at least one honest human approver per account, a trace validator key held under the operational model documented in Section \ref{sec:onchain-layer}, and a host chain whose consensus layer is not actively under attack at the moment we settle. If any of these break, the safety properties of Section \ref{sec:proof-targets} do not apply. We do not assume that the language model is correct, that any single evidence source is honest, that the off-chain runtime is uncompromised, or that the user's stated intent is the one they actually had in mind.
...
Figure \ref{fig:icvp-formal-flow} renders the formal pipeline. Intent $i$ feeds three operators ($P_i$, $\Phi_i$, $g_i$), each rewriting a different stage of the pipeline, and the trace $\tau$ aggregates evidence, instantiated dimensions, and the decision at the end.

\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth,height=0.72\textheight,keepaspectratio]{images/icvp-formal-flow.png}
    \caption{Formal flow of ICVP: from asset, evidence, and intent through projection $P_i$, instantiation $\Phi_i$, and aggregation $g_i$ to the decision $d$ and trace $\tau$.}
    \label{fig:icvp-formal-flow}
\end{figure}
...
The valuation trace $\tau$ is stored off-chain in a content-addressed store, addressed by $\textit{cid}(\tau) = \textsc{Hash}(\tau)$. IPFS is the reference choice in the prototype, but any content-addressed store with the same integrity property is admissible. Storing $\tau$ off-chain keeps the on-chain footprint small, and storing it content-addressed makes the on-chain hash a binding commitment to the bytes that the trace validator and the human approver saw.

\paragraph{Schema commitment and trace validator signature.}
The trace validator signs the trace jointly with its schema version and schema hash, not separately. Let $\textit{ver}(\tau)$ be the schema version that $\tau$ was validated against and let $\textit{sh}(\tau) = \textsc{Hash}(\textit{schema}_{\textit{ver}(\tau)})$ be the content hash of that schema. The trace validator emits
\[
\sigma_V \;=\; \textsc{Sign}_{\textit{sk}_V}\bigl(\textsc{Hash}(\tau)\,\|\,\textit{ver}(\tau)\,\|\,\textit{sh}(\tau)\bigr).
\]
Joint signing has the property that an attempt to retroactively weaken the schema breaks $\sigma_V$, because $\textit{sh}(\tau)$ would no longer match the signed value. This contains threat T10 from the threat register in Section \ref{sec:threat-model}. The trace validator key $\textit{sk}_V$ is documented separately. A single trace validator key is acceptable for the prototype and is the configuration we use. The associated trust assumption is that $\textit{sk}_V$ is held in a way consistent with the trust assumptions of Section \ref{sec:threat-model}. A multi-validator or threshold configuration of the trace validator is a straightforward extension and does not change the safety statements.
...
\]
Joint signing has the property that an attempt to swap the transaction after approval breaks $\sigma_H$, because $\textsc{Hash}(\textit{tx})$ would no longer match the signed value. This contains threat T7 from the threat register in Section \ref{sec:threat-model}.
...
\begin{enumerate}
    ...
    \item \textbf{Trace validator signature.} $\textsc{Verify}_{\textit{pk}_V}(\sigma_V, \textsc{Hash}(\tau)\,\|\,\textit{ver}(\tau)\,\|\,\textit{sh}(\tau))$ holds. (Realises $\textsc{ValidTrace}$ jointly with the previous step.)
...
\end{enumerate}
Settlement happens only when all eight checks hold. If any check fails the call reverts and no state change is committed. The freshness predicate $\textsc{FreshEvidence}(\tau, t)$ is enforced as part of step seven on the NFTAA side, because $\textit{exp}(\tau)$ is computed off-chain as the minimum of the per-category TTL horizons and is recorded in $\tau$ before trace validator signing. An attempt to relax $\textit{exp}(\tau)$ after signing breaks $\sigma_V$. The eight checks together realise the Trace-Gated Authorisation property at the execution boundary. The detailed call sequence for the prototype, including the signing pipeline and the NFTAA call sites, is presented in the implementation chapter alongside the contract code.
...
We propose the following setup for asset valuation in agentic systems within the Internet of Value:
...
The following subsection describes the technical and operational requirements for a functional architecture for agentic systems in the context of the Internet of Value and NFTs. The requirements are organized into four categories: data acquisition, agent capabilities, execution guarantees, and security properties.
...
\begin{itemize}
    \item The system should retrieve data from at least one data source, such as blockchain explorers, decentralized storage networks, marketplace APIs, rarity analytics, or oracle services.
    ...
    \item Each data point should be accompanied by provenance information, including the source URL, retrieval timestamp, and content hash. This supports verifiability and auditability of the acquired information.
\end{itemize}
...
\begin{itemize}
    ...
    \item Produce a structured valuation trace, including a decision in $\{\textit{proceed}, \textit{defer}, \textit{decline}\}$, that a deterministic trace validator can check for required evidence, provenance, and policy compatibility.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth,height=0.72\textheight,keepaspectratio]{images/architecture-new.png}
    \caption{Architecture of the agentic system: four areas (local, agentic, external data, on-chain) with the valuation trace $\tau$ as the central artefact and human approval as an explicit gate before NFTAA-enforced settlement.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth,height=0.62\textheight,keepaspectratio]{images/icvp-valuation-internal.png}
    \caption{Internal view of the valuation component: evidence retrieval via MCP adapters, projection $P_i$, instantiation $\Phi_i$, and parallel emission of $A_i$, $D_i$, $U_i$ with the insufficient-evidence path.}
...
Figure \ref{fig:icvp-domain-model} ties these fields back to the broader vocabulary of the architecture. The conceptual model shows how Intent, Asset, Projection, Evidence with Provenance, the instantiated Dimensions, the Trace, the Decision, Human Approval, the Candidate Transaction, NFTAA, and Policy relate. The trace is the central entity that links evidence interpretation on one side to human approval and on-chain enforcement on the other.

\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth,height=0.72\textheight,keepaspectratio]{images/icvp-domain-model.png}
    \caption{Conceptual model of the core entities and their relationships.}
    \label{fig:icvp-domain-model}
\end{figure}
...
First, instead of depending on a strict, registry-based deployment architecture, NFTAA enables the integration of specific semantic rules directly into the token contract. ERC-6551 limits the design space for specific behaviours by deploying each token-bound account via a singleton registry with a deterministic address scheme. By enabling developers to create custom account logic inside the NFT contract itself, NFTAA eliminates this restriction and provides fine-grained control over state transitions, access patterns, and execution policies.
