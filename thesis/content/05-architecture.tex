...
The architecture combines components with very different trust properties. The on-chain layer is deterministic and inspectable. The validator and the schemas are authored artefacts under our control. The language model that drives the proposer is opaque and replaceable. External evidence sources are partially trusted and can fail or be manipulated. Human approvers are trusted to authorise their own actions but are not infallible. A defensible safety story therefore has to name the adversaries and failure modes that the design contains, identify which components are trusted and which are not, and connect each row to the mechanism that contains it. We do this in Table \ref{tab:threat-model}. The safety properties in Section \ref{sec:proof-targets} are stated against this table, and each row is referenced from at least one invariant or check.
...
\hline
T2 & Stale evidence (data that was fresh when retrieved but is no longer fresh at authorisation time) & Evidence sources are untrusted, validator is trusted & Per-category freshness TTLs in the \textit{RequiredEvidence} schema, evaluated at authorisation time $t$, not at trace construction time. Section \ref{sec:proof-targets} states this as part of the Trace-Gated Authorisation property. \\
\hline
...
\hline
\end{tabular}
\caption{Threat model for the ICVP architecture. Each row names a threat, the trust status of the affected component, and the mechanism that contains it. Safety properties in Section \ref{sec:proof-targets} reference the rows they depend on.}
\label{tab:threat-model}
\end{table}

Three trust assumptions stand outside the table because they bound the model rather than appear as rows in it. We assume that at least one human approver acts honestly for the actions submitted through their account, that the validator key is held in a way consistent with its documented model in Section \ref{sec:onchain-layer}, and that the consensus layer of the host chain is not under active attack at the moment of settlement. We do not assume that the language model is correct, that any individual evidence source is honest, that the off-chain runtime is uncompromised, or that intents are well-posed.
