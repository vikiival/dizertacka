...
Each blockchain stores different data in the block for correct functioning. This data depends on the selected consensus algorithm or the implementation of the blockchain.
...
\begin{itemize}
  \item[ ] Proof-of-Work (PoW) - The idea behind Proof-of-Work is that to make a valid block, we need to use extensive computational resources to find a correct value in the cryptographic puzzle.
  ...
  Therefore, the block reward is split between the validator and other nominators by the ratio of their stake. NPoS is used in Polkadot and its parachains.
\end{itemize}
...
The term \emph{smart contract} predates blockchain, and Bitcoin Script already supported limited contract logic. Ethereum was the first blockchain to support Turing-complete smart contracts at the protocol level. In high overview, the Ethereum blockchain has an Ethereum Virtual Machine (EVM) that is used to run smart contracts.
Thanks to the Turing completeness of the EVM, when we want to write a smart contract, we can use one of the high-level programming languages like Solidity or Vyper \cite{antonopoulos2018mastering}.
...
To make the computation sustainable, when we want to make a transaction, we need to pay a fee defined by the gas \cite{Leonardos2021, Roughgarden2020}. Gas serves as the fee for miners to include the transaction in a block. Additionally, a tip can be included for faster transaction processing.
...
\begin{table}[ht]
  ...
  \hline
  Indicative price & per-GB monthly overage pricing \cite{PinataPricing2026} & per-TB monthly storage tiers \cite{StorjPricing2026} & one-time permanent-storage pricing \cite{ArDrivePricing2026}     \\
  \hline
  \end{tabular}
